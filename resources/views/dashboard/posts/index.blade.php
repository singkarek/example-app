...
@section('isi')
    ...
    @endif
    <a href="/dashboard/posts/create" class="p-2 bg-green-400 hover:bg-green-500 text-sm rounded-xl">Create new post</a>
                ...
                @foreach ($posts as $post)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-6 py-4 flex gap-2">
                            <a href="/dashboard/posts/{{ $post->slug }}" class="p-1 rounded-md bg-green-500 hover:bg-green-400">
                                <svg class="w-4 h-4 text-gray-800" aria-hidden="true"
                                    xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 14">
                                    <path
                                        d="M10 0C4.612 0 0 5.336 0 7c0 1.742 3.546 7 10 7 6.454 0 10-5.258 10-7 0-1.664-4.612-7-10-7Zm0 10a3 3 0 1 1 0-6 3 3 0 0 1 0 6Z" />
                                </svg></a>
                            <a href="/dashboard/posts/{{ $post->slug }}/edit" class="p-1 rounded-md bg-yellow-500 hover:bg-yellow-400">
                            ...
                            <form action="/dashboard/posts/{{ $post->slug }}" method="POST">
                                @method('delete')
                                @csrf
                                <button class="p-1 rounded-md bg-red-500 hover:bg-red-400" onclick="return confirm('Are you sure?')">
                                    <svg class="w-4 h-4 text-gray-800" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
